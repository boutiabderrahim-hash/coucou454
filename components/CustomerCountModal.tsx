import React, { useState } from 'react';
import { UserGroupIcon } from './icons';

interface CustomerCountModalProps {
    isOpen: boolean;
    onClose: () => void;
    onConfirm: (count: number) => void;
}

const CustomerCountModal: React.FC<CustomerCountModalProps> = ({ isOpen, onClose, onConfirm }) => {
  const [count, setCount] = useState('');

  if (!isOpen) return null;

  const handleConfirm = () => {
    const countValue = parseInt(count, 10);
    if (!isNaN(countValue) && countValue > 0) {
      onConfirm(countValue);
    } else {
      alert("Por favor, introduzca un número válido de clientes.");
    }
  };

  const handleNumpad = (value: string) => {
    if (value === 'del') {
      setCount(c => c.slice(0, -1));
    } else {
      if (count.length < 3) {
        setCount(c => c + value);
      }
    }
  }

  return (
    <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50">
      <div className="bg-gray-800 rounded-2xl shadow-2xl p-8 w-full max-w-sm">
        <h2 className="text-3xl font-bold text-white mb-4 text-center">Número de Clientes</h2>
        
        <div className="relative mb-6">
          <UserGroupIcon className="absolute left-4 top-1/2 -translate-y-1/2 h-8 w-8 text-gray-400" />
          <input
            type="text"
            value={count}
            readOnly
            placeholder="0"
            className="w-full bg-gray-900 border-2 border-gray-700 rounded-lg py-4 pl-16 pr-4 text-4xl text-right font-mono focus:outline-none focus:ring-2 focus:ring-indigo-500"
            autoFocus
          />
        </div>

        <div className="grid grid-cols-3 gap-2 mb-6">
            {['1', '2', '3', '4', '5', '6', '7', '8', '9'].map(key => (
                 <button key={key} onClick={() => handleNumpad(key)} className="text-2xl font-bold bg-gray-700 rounded-lg py-4 hover:bg-gray-600 transition-colors">
                    {key}
                </button>
            ))}
             <div/>
             <button onClick={() => handleNumpad('0')} className="text-2xl font-bold bg-gray-700 rounded-lg py-4 hover:bg-gray-600 transition-colors">0</button>
             <button onClick={() => handleNumpad('del')} className="text-2xl font-bold bg-gray-700 rounded-lg py-4 hover:bg-gray-600 transition-colors">⌫</button>
        </div>

        <div className="flex justify-between space-x-4">
          <button onClick={onClose} className="w-full py-3 bg-gray-600 rounded-lg font-semibold hover:bg-gray-700 transition-colors">
            Cancelar
          </button>
          <button onClick={handleConfirm} className="w-full py-3 bg-green-600 rounded-lg font-semibold hover:bg-green-700 transition-colors">
            Confirmar
          </button>
        </div>
      </div>
    </div>
  );
};

export default CustomerCountModal;